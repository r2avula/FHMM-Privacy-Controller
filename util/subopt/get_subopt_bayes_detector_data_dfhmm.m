function [bayesDetectorData] = get_subopt_bayes_detector_data_dfhmm(params,max_valueFnIterations)
...
lP_DXkgHkHkn1 = log(P_DXkgHkHkn1);
lP_Hk_vec_gHkn1_vec = log(P_Hk_vec_gHkn1_vec);

bayesDetectorData_filePrefix = 'cache/bayesDetectorData_dfhmm_';
...
if(fileExists)
    ...
    inst_reward = minLogParam*ones(h_vec_num,Dx_num,h_vec_num,horizonsPerDay);
    parfor hh_vec_kn1_idx = 1:h_vec_num
        lP_DXkgHkHkn1_t = lP_DXkgHkHkn1(:,:,hh_vec_kn1_idx);
        lP_Hk_vec_gHkn1_vec_t = lP_Hk_vec_gHkn1_vec;
        possible_h_vec_idxs_t = possible_h_vec_idxs{hh_vec_kn1_idx};
        inst_reward_t = minLogParam*ones(h_vec_num,Dx_num,1,horizonsPerDay);
        for horizonIdx = 1:horizonsPerDay
            for dx_k_idx = 1:Dx_num
                for hh_vec_k_idx = possible_h_vec_idxs_t
                    inst_reward_t(hh_vec_k_idx,dx_k_idx,1,horizonIdx) = max(lP_DXkgHkHkn1_t(dx_k_idx,hh_vec_k_idx)+lP_Hk_vec_gHkn1_vec_t(hh_vec_k_idx,hh_vec_kn1_idx,horizonIdx),minLogParam);
                end
            end
        end
    ...
    valueFunction_in1 = zeros(Dx_num,h_vec_num,horizonsPerDay);
    for iter_idx  = 1:max_valueFnIterations
        fprintf('\t\tPerforming value iteration step : %d - ',iter_idx);
        valueFunction_i = minLogParam*ones(Dx_num,h_vec_num,horizonsPerDay);
        det_strategy_i = zeros(Dx_num,h_vec_num,horizonsPerDay);
            ...
            for horizonIdx = 1:horizonsPerDay
                ...
                for h_vec_kp1_idx = possible_h_vec_idxs_t
                    temp = temp + P_Hk_vec_gHkn1_vec_t(h_vec_kp1_idx,hh_vec_k_idx,horizonIdx)*...
                        P_DXkgHkHkn1_t(:,h_vec_kp1_idx,hh_vec_k_idx)'*discounted_approxValueFn_in1_t(:,hh_vec_k_idx,horizonIdx);
                end
        ...
        parfor hh_vec_kn1_idx = 1:h_vec_num
            ...
            det_strategy_i_t = zeros(Dx_num,1,horizonsPerDay);
            valueFunction_i_t = minLogParam*ones(Dx_num,1,horizonsPerDay);
            
            for horizonIdx = 1:horizonsPerDay
                for dx_k_idx = 1:Dx_num
                    value_func_t = inst_reward_t(:,dx_k_idx,1,horizonIdx) + exp_discounted_approxValueFn_in1_t(:,horizonIdx);
                    [max_val,opt_hh_vec_k_idx] = max(value_func_t);
                    valueFunction_i_t(dx_k_idx,1,horizonIdx) = max_val;
                    det_strategy_i_t(dx_k_idx,1,horizonIdx) = opt_hh_vec_k_idx;
                end
            end
