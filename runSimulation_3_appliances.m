...
opt_bayesian_control_hyp_unaware = 1;
subopt_bayesian_control_hyp_unaware = 1;
config_filename = '02_fridge_tv_stereo_600s_75w_30ah.yaml';
...
optimal_control_viterbi_detection_accuracy = zeros(1,appliances_num);
...
if(opt_bayesian_control_hyp_unaware)
    disp('Optimal Bayesian hyp. unaware controller with discrete belief space --- ');    
    ...
    fileNamePrefix = 'cache/evalData_hua_bayesian_control_';
    [fileName,fileExists] = findFileName(evalParams,fileNamePrefix,'evalParams');
    if(fileExists)
        fprintf(['\t\tEvaluation skipped. Data found in: ',fileName,'\n']);
        load(fileName,'det_accuracy_mean_viterbi','detected_sequence_viterbi');
    else        
        bayesDetectorParams.beliefSpacePrecision = params.beliefSpacePrecision;
        [bayesDetectorData] = get_opt_bayes_detector_data(bayesDetectorParams);        
        [policy] = get_opt_hyp_aware_policy(params,bayesDetectorData,max_valueFnIterations);        
        [controlledData] = simulate_hua_bayesian_control(evalParams,bayesDetectorData,policy,smdata);
    ...
    optimal_control_viterbi_detection_accuracy(:) = det_accuracy_mean_viterbi;
    detected_sequence_vec = reshape(detected_sequence_viterbi,[],1);
    risk_mean_viterbi =  zeros(1,private_appliance_num);
    for p_idx = 1:private_appliance_num
        app_idx = private_appliance_idxs(p_idx);
