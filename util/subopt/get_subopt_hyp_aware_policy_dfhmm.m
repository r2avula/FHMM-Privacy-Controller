function [policy] = get_subopt_hyp_aware_policy_dfhmm(params,bayesDetectorData,max_valueFnIterations)
...
stat_policy_fileNamePrefix = 'cache/stat_subopt_policy_ha_dfhmm_';
...
if(fileExists)
    ...
    valueFunction_in1 = zeros(x_num,z_num,y_num,h_vec_num,horizonsPerDay,h_vec_num);    
    for iter_idx  = 1:max_valueFnIterations
        fprintf('\t\tPerforming value iteration step : %d - ',iter_idx);
        valueFunction_i = maxCostParam*ones(x_num,z_num,y_num,h_vec_num,horizonsPerDay,h_vec_num);
        emu_strategy_i = zeros(x_num,z_num,y_num,h_vec_num,horizonsPerDay,h_vec_num);
                            ...
                            for h_vec_k_idx = 1:h_vec_num
                                ...
                                for h_vec_kp1_idx = possible_h_vec_idxs_t{h_vec_k_idx}
                                    temp = temp + P_Hk_vec_gHkn1_vec_t(h_vec_kp1_idx,h_vec_k_idx,horizonIdx)*...
                                        P_DXkgHkHkn1_t((1:x_num)-x_k_idx-Dx_offset,h_vec_kp1_idx,h_vec_k_idx)'*discounted_approxValueFn_in1_t(:,:,y_k_idx,h_vec_kp1_idx,horizonIdx,hh_vec_k_idx)*z_kp1_idx_prob;
                                end
                    ...
                    for x_k_idx = 1:x_num
                        for h_vec_k_idx = 1:h_vec_num
                            for y_kn1_idx = 1:y_num
                                ...
                                for y_k_idx = valid_y_idxs_t{x_k_idx,z_kn1_idx}
                                    hh_vec_k_idx = det_strategy_t(y_k_idx-y_kn1_idx-Dx_offset,hh_vec_kn1_idx,horizonIdx);
        ...
        if(max_val_inc<=params.min_max_val_inc || iter_idx==max_valueFnIterations)     
            ...
            parfor h_vec_k_idx = 1:h_vec_num
                ...
                emu_strategy_1_t = zeros(x_num,z_num,1,horizonsPerDay);
                ...
                for horizonIdx = 1:horizonsPerDay
                    for z_kn1_idx = 1:z_num
                        for x_k_idx = 1:x_num
                            ...
                            emu_strategy_1_t(x_k_idx,z_kn1_idx,1,horizonIdx) = opt_y_idx;
                        end
                    end
                end
                
                emu_strategy_1(:,:,h_vec_k_idx,:) = emu_strategy_1_t;
                send(parforProgress, incPercent);
            end
